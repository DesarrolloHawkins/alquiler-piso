    ...
    $(document).ready(function() {
        ...
        tipoDocumento.forEach(function(tipo){
            tipo.addEventListener('change', function(e) {
                ...
                setFileInputRequirements(valor, info);
            });
        });

        // Ejecutar la lógica al cargar la página para configurar correctamente los campos required
        tipoDocumento.forEach(function(tipo) {
            var valor = tipo.value;
            var info = tipo.getAttribute('data-info');
            setFileInputRequirements(valor, info);
        });
    });

    function setFileInputRequirements(valor, info) {
        if (valor === '1') {
