...
<div class="container-fluid">
    <form action="{{ route('fotos.banioStore', $id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="files mt-4 card p-2">
            ...
            <input type="file" accept="image/*" class="file-input" capture="camera" name="image_inodoro" id="image_inodoro" onchange="previewImage2(event)">
...
<script>
    ...
    window.onload = function() {
        ...
        var imageUrl = "{{ $imageUrlInodoro }}";
        if (imageUrl) {
            var output = document.getElementById('image-preview2');
            output.src = imageUrl;
            output.style.display = 'block';
        }
        var imageUrl = "{{ $imageUrlDesague }}";
        if (imageUrl) {
